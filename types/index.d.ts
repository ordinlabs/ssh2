...
import {
    Agent as BaseHTTPAgent,
    AgentOptions as BaseHTTPAgentOptions,
} from "http";
import {
    Agent as BaseHTTPSAgent,
    AgentOptions as BaseHTTPSAgentOptions,
} from "https";
...
declare module "@ordinlabs/ssh2" {
     ...
     */
    export type AlgorithmList<T> =
        | T[]
        | Record<"append" | "prepend" | "remove", T | T[]>;
    ...
    export namespace utils {
        ...
        function generateKeyPairSync<K extends KeyType>(
            keyType: K,
            opts: KeySettings[K] & KeyPairOptions
        ): KeyPairReturn;
    ...
    export interface ClientChannel extends Channel {
        ...
        on(
            event: "exit",
            listener: (code: null, signal: string, dump: string, desc: string) => void
        ): this;
        on(event: string | symbol, listener: Function): this;
    ...
    export type HostFingerprintVerifier = (
        fingerprint: string,
    ...
    export type NextAuthHandler = (
        authName: AuthenticationType | AnyAuthMethod
    ) => void;
    ...
    export interface Connection extends EventEmitter {
        ...
        once(
            ...
            listener: (context: AuthContext) => void
        ): this;
    ...
    export interface Session extends ServerChannel {
        ...
        on(
            event: "sftp",
            listener: (accept: AcceptSftpConnection, reject: RejectConnection) => void
        ): this;
    ...
    export interface SFTPWrapper extends EventEmitter {
        ...
        on(
            event: "READDIR",
            listener: (reqId: number, handle: Buffer) => void
        ): this;
    ...
    export type KnownPublicKeys<
        T extends string | Buffer | ParsedKey = string | Buffer | ParsedKey
    > = Array<T | PublicKeyEntry>;

    export type PrivateKeys = Array<
        Buffer | ParsedKey | EncryptedPrivateKey | string
    >;
    ...
    export class AgentProtocol extends Duplex {
        ...
        once(
            event: "identities",
            listener: (req: AgentInboundRequest) => void
        ): this;
    ...
    export type ClientForwardCallback = (
        err: Error | undefined,
        port: number
    ) => void;

    export type ClientSFTPCallback = (
        err: Error | undefined,
        sftp: SFTPWrapper
    ) => void;
