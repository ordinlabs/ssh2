 ...
 * undefined if more data is needed, or null if not a PROXY protocol header.
 */
function parseProxyHeader(data) {
  // PROXY protocol v1: starts with "PROXY " (ASCII)
  if (data.length >= 8 && data.toString('ascii', 0, 6) === 'PROXY ') {
    const crlfIdx = data.indexOf('\r\n');
    if (crlfIdx === -1) return undefined; // Need more data
  ...
  if (data.length >= 16 && data.subarray(0, 12).equals(PROXY_V2_SIG)) {
    const verCmd = data[12];
    const version = (verCmd & 0xF0) >> 4;
    const command = verCmd & 0x0F;

    if (version !== 2) return null; // Not v2

    const addrLen = data.readUInt16BE(14);
    const totalLen = 16 + addrLen;

    if (data.length < totalLen) return undefined; // Need more data
