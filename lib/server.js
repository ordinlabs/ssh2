...
const PROXY_V2_SIG = Buffer.from([
  0x0D, 0x0A, 0x0D, 0x0A, 0x00, 0x0D, 0x0A, 0x51,
  0x55, 0x49, 0x54, 0x0A
]);

/**
 * Parse PROXY protocol header (v1 or v2) from the beginning of a buffer.
 * Returns { ip, port, family, remaining } if a valid header is found,
 * null if more data is needed, or false if not a PROXY protocol header.
 */
function parseProxyHeader(data) {
  if (data.length === 0) return null;

  // PROXY protocol v1: starts with "PROXY " (ASCII)
  const proxyV1Prefix = 'PROXY ';
  if (data.length < 8) {
    // Check if what we have so far could be the start of v1
    if (proxyV1Prefix.startsWith(data.toString('ascii', 0, Math.min(data.length, 6))))
      return null;
  } else if (data.toString('ascii', 0, 6) === proxyV1Prefix) {
    const crlfIdx = data.indexOf('\r\n');
    if (crlfIdx === -1) return null; // Need more data

    const headerLine = data.toString('ascii', 0, crlfIdx);
    ...
    if (parts.length >= 6 && (parts[1] === 'TCP4' || parts[1] === 'TCP6')) {
      return {
        ...
        family: parts[1] === 'TCP4' ? 'IPv4' : 'IPv6',
        remaining: data.subarray(crlfIdx + 2),
      };
    }
    // UNKNOWN protocol — use socket defaults
    return {
      ip: null,
      port: null,
      family: null,
  ...
  // PROXY protocol v2: starts with 12-byte signature
  if (data.length < 16) {
    // Check if what we have so far could be the start of v2
    if (data.subarray(0, Math.min(data.length, 12)).equals(PROXY_V2_SIG.subarray(0, Math.min(data.length, 12))))
      return null;
  } else if (data.subarray(0, 12).equals(PROXY_V2_SIG)) {
    const verCmd = data[12];
    const version = (verCmd & 0xF0) >> 4;
    const command = verCmd & 0x0F;

    if (version !== 2) return false; // Not v2

    const addrLen = data.readUInt16BE(14);
    const totalLen = 16 + addrLen;

    if (data.length < totalLen) return null; // Need more data
    ...
    if (command === 0) {
      return {
        ...
        port: null,
        family: null,
        remaining: data.subarray(totalLen),
      };
    }

    const addrFamily = (data[13] & 0xF0) >> 4;

    if (addrFamily === 1) {
      // AF_INET (IPv4)
      const srcIP = `${data[16]}.${data[17]}.${data[18]}.${data[19]}`;
      const srcPort = data.readUInt16BE(24);
      return {
        ip: srcIP,
        port: srcPort,
        family: 'IPv4',
      ...
      for (let i = 0; i < 16; i += 2) {
        parts.push(srcAddr.readUInt16BE(i).toString(16));
      }
      const srcIP = parts.join(':');
      const srcPort = data.readUInt16BE(48);
      return {
        ip: srcIP,
        port: srcPort,
        family: 'IPv6',
    ...
    // Unknown address family — skip header, use socket defaults
    return {
      ip: null,
      port: null,
      family: null,
...
class Server extends EventEmitter {
  constructor(cfg, listener) {
    ...
    const proxyProtocol = cfg.proxyProtocol === true;
    ...
    this._srv = new netServer((socket) => {
      ...
      // PROXY protocol enabled — intercept first bytes to parse the header
      ...
      const onFirstData = (data) => {
        ...
        if (!proxyChecked) {
          const result = parseProxyHeader(buffered);

          if (result === null) {
            // Need more data to determine if it's PROXY protocol
            return;
          }
          ...
          if (result !== false) {
            // PROXY header found — override socket address properties
            if (result.ip) {
              // Preserve the original (proxy) address before overriding
              Object.defineProperty(socket, '_proxyAddress', {
                value: socket.remoteAddress,
                writable: false,
                configurable: true,
              });
              Object.defineProperty(socket, '_proxyPort', {
            ...
            debug && debug(`PROXY protocol: real client ${result.ip}:${result.port}`);
            buffered = result.remaining;
          } else {
            // Not a PROXY header — buffered contains the original SSH data
            debug && debug('PROXY protocol enabled but no PROXY header detected');
          }

          // eslint-disable-next-line no-use-before-define
          const client = new Client(
            socket, hostKeys, ident, offer, debug, this, cfg
          );

          // The Client constructor calls socket.pause() synchronously, then
          // sets up its data listener after cryptoInit resolves. Use unshift()
          // to push buffered data back into the readable stream so it will be
          // read when the Client resumes the socket.
      ...
      // Timeout for PROXY header detection — if we don't get enough
      // data within 5 seconds, assume it's not PROXY protocol
      const proxyTimeout = setTimeout(() => {
        if (!proxyChecked) {
          proxyChecked = true;
          socket.removeListener('data', onFirstData);
          debug && debug('PROXY protocol detection timed out');
          // eslint-disable-next-line no-use-before-define
          const client = new Client(
            socket, hostKeys, ident, offer, debug, this, cfg
          );
    ...
    const proto = this._protocol = new Protocol({
      ...
      onHeader: (header) => {
        ...
        const info = {
          ...
          header,
          proxyAddress: socket._proxyAddress || undefined,
          proxyPort: socket._proxyPort || undefined,
        };
