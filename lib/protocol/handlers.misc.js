    ...
    switch (method) {
        ...
        if (hasSig !== undefined) {
          ...
          if (hasSig) {
            ...
            if (signature !== undefined) {
              // RFC 8332: The signature blob contains its own algorithm
              // identifier which may differ from keyAlgo. For example, a client
              // may put 'ssh-rsa' in the packet algorithm field but sign with
              // 'rsa-sha2-256'. Read the embedded algorithm to strip the header
              // correctly and derive the hash algorithm.
              if (signature.length > 8) {
                const sigAlgoLen = signature.readUInt32BE(0);
                if (sigAlgoLen > 0 && sigAlgoLen < 64
                    && signature.length > (4 + sigAlgoLen + 4)) {
                  const sigAlgo = signature.utf8Slice(4, 4 + sigAlgoLen);
                  if (sigAlgo === keyAlgo
                      ...
                      || sigAlgo === 'ssh-dss'
                      || sigAlgo === 'ssh-ed25519'
                      || sigAlgo.startsWith('ecdsa-sha2-')) {
                    signature = bufferSlice(signature, 4 + sigAlgoLen + 4);
                    // Derive hashAlgo from the signature's algorithm if the
                    // packet-level algorithm didn't already set it
                    if (!hashAlgo) {
                      switch (sigAlgo) {
      ...
      case 'hostbased': {
        ...
        */
        ...
        let signature = bufferParser.readString();
        if (signature !== undefined) {
          // RFC 8332: Same unwrapping logic as publickey auth — the signature
          // blob may contain a different algorithm than the packet-level keyAlgo.
          if (signature.length > 8) {
            const sigAlgoLen = signature.readUInt32BE(0);
            if (sigAlgoLen > 0 && sigAlgoLen < 64
                && signature.length > (4 + sigAlgoLen + 4)) {
